import { sanitizePrice } from '@utils/formatters'
import z from 'zod'

export const schemaCostUpdate = z.object({
  id: z.string().min(1, 'É necessário o id para a atualização'),
  name: z.string().refine(value => value.length >= 5, 'A descrição deve conter mais de 5 caracteres'),
  amount: z.string().refine(value => sanitizePrice(value) > 0, 'Informe um valor maior que R$ 0,00'),
  categoryId: z.string().refine(value => !!value, 'Informe a categoria')
})
